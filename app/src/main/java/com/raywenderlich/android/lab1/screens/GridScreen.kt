...
@Composable
fun GridView(columnCout: Int){ val itemSize = items.size
    val rowCount = ceil(itemSize.toFloat() / columnCout).toInt()
    val gridItems = mutableListOf<List<IconResource>>()
    var position = 0
    for (i in 0 until rowCount){
        val rowItem = mutableListOf<IconResource>()
        for (j in 0 until  columnCout){
            if (position.inc() <= itemSize){
                rowItem.add(IconResource(items[position++], true))
            }
        }
    }
    val itemsToFill = columnCout - rowItem.size
